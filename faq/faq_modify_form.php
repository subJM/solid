	...
	<section>
		<div id="board_box">
			...
			if(isset($_GET["num"])){
				...
				mysqli_close($con);
				header("location: faq_list.php?error=수정하기 오류발생!");
				exit(); 
			}	
			// print_r($row);
			?>
			<form name="board_form" method="post" action="faq_update.php?num=<?= $num ?>" enctype="multipart/form-data">
				<ul id="board_form">
					<li>
			...
			<ul class="buttons">
				...
				?>
					...
					<li><button type="button" onclick="location.href='faq_delete.php?num=<?= $num ?>'">삭제</button></li>
