...
$sql = " SELECT a.*, b.id, b.name, b.email
            FROM memo a
            LEFT JOIN members b ON (a.me_{$unkind}_mb_id = b.id)
...
for ($i = 0; $row = mysqli_fetch_assoc($result); $i++) {
	...
	$list[$i]['mb_name'] = $row["me_{$unkind}_mb_id"];
				...
				<tbody>
					<?php for ($i = 0; $i < count($list); $i++) {  ?>
						<tr>
							...
							<td><a href="<?php echo $list[$i]['view_href'] ?>"><?php echo $list[$i]['read_datetime'] ?></a>
							</td>
							<td><a href="<?php echo $list[$i]['del_href'] ?>" onclick="del(this.href); return false;">삭제</a>
							</td>
						</tr>
					<?php }  ?>
					<?php if ($i == 0) {
