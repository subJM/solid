<?php
    ...
    if(isset($_POST["subject"]) && isset($_POST["content"])){
        ...
        //4. 공백이 있는지 점검
        if(empty($subject)){
            header("location: notice_form.php?error=제목을 입력해주세요!");
            exit;
        }else if(empty($content)){
            header("location: notice_form.php?error=내용을 입력해주세요!");
            exit;
        }else{
            // echo "아이디확인2 <br>" ;
            ...
            if(mysqli_num_rows($select_result) === 0){
                // echo "아이디확인1 <br>" ;
                header("location: notice_form.php?error=아이디가 없습니다!");
                exit;
            }else{
                // echo "아이디확인 <br>" ;
                ...
                if(!$insert_result){
                    ...
                    mysqli_close($con);
                    header("location: notice_form.php?error=글등록에 실패했습니다!");
                    exit();
                }else{
                    // echo "insert 등록 성공 <br>" ;
                    mysqli_close($con);
                    header("location: notice_list.php?success=글등록에 성공했습니다!");
                    exit();
                }
